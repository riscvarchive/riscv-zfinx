...
`Zfinx` changes all existing and future floating point extensions that use the `F` floating point registers, so that instead they use the `X` registers. Hence the name `F-in-X`. This does not affect floating point instructions that are implemented as part of the Vector (`v`) extension. `Zfinx` additionally removes all:
