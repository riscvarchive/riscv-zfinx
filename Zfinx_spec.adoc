...
NaN boxing is useful in cases where it is unknown whether floating point data will be interpreted as the full width of the register, or as a shorter value. For example for RV32F Zfh *Zfinx* when a half-precision value is passed as a function argument, 
